...
- [x] ESLint config를 browser, Node/CommonJS, test, extension 경계로 나누고 generated/worktree 경로를 ignore한다.
- [x] 기존 1,354 lint 오류를 환경 오류, 실제 correctness 오류, compiler advisory로 분류해 실제 오류를 수정하고 gate를 0 error로 만든다.
- [x] React 19.2.8, Vite 7.3.6, Mermaid 11.16.1 등 현재 major의 안전 패치를 명시적으로 적용한다.
...
- [x] quality workflow가 install, lint, unit/API, Playwright, web/extension build, audit를 실행하게 한다.
...
- [x] Actions를 최소 권한과 immutable SHA로 고정한다.
- [x] 품질 게이트와 native release 변경을 Conventional Commit으로 분리해 기록한다.
...
- [x] Node 24.15를 local engine, CI, build/runtime image에 통일한다.
- [x] nginx의 `/health`가 API health를 확인하고 API 종료 시 readiness가 실패하는 container test를 작성한다.
- [x] Node와 nginx에 종료 신호가 전달되는 init/supervision entrypoint를 적용한다.
- [x] upload route별 nginx 제한과 현대적 보안 header를 적용한다.
- [x] 저장소 `k8s/`가 non-authoritative임을 명시하고 라이브 GitOps SSOT를 문서화한다.
...
- [x] Docker build/run smoke를 실행한다.
- [x] 런타임과 운영 변경을 Conventional Commit으로 분리해 기록한다.
...
- [x] plan/spec 요구사항을 diff와 테스트 목록에 대조한다.
- [x] fresh reviewer가 security, compatibility, data-loss, deployment findings를 검토하고 발견된 Critical/Important를 수정한다.
- [x] `npm run verify`, 세 전체 dependency audit, 두 build, API test, Docker smoke를 새로 실행한다.
- [x] live secret, NAS CA/hostname, PVC JSON/token migration readiness를 read-only preflight한다.
- [x] 이번 modernisation history에는 `feat: add native release planner and publisher`가 있으므로 다음 release가 minor `1.52.0`임을 확인한다. 과거 계획의 patch baseline은 당시의 historical assumption으로만 보존한다.
